...
describe('Link checks', () => {
  ...
  describe('a()', () => {
    it('should say true for a ref of that type', () => {
      expect(a(Parameter)(Ref(Parameter('sarasa')))).to.equals(true)
    })
    it('should say false for a ref of another type', () => {
      expect(a(Parameter)(Ref(Field('sarasa')))).to.equals(false)
    })
  })

  describe('many()', () => {
    it('should say true if all elements comply', () => {
      expect(many(Parameter)([Ref(Parameter('p1')), Ref(Parameter('p2'))])).to.equals(true)
    })
    it('should say false if none comply with the type', () => {
      expect(many(Parameter)([Ref(Field('p1')), Ref(Field('p2'))])).to.equals(false)
    })
    // this case is still like.. under-designed
    it('should say false if one of them does NOT comply', () => {
      expect(many(Parameter)([Ref(Parameter('p1')), Ref(Field('p2'))])).to.equals(false)
    })
  })
